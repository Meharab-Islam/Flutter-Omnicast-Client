...
void main() {
  ...
  group('RoomMode & PkScore State Management', () {
    ...
    test('updating PK battle transitions RoomMode to pk and sets PkScore', () {
      ...
      expect(
        roomState.pkScoreNotifier.value.hostRatio,
        closeTo(100 / 350, 0.01),
      );
    });
  ...
  group('OmniCastVideoCanvas Widget', () {
    testWidgets(
      'renders solo layout by default and switches to PK split-screen on state change',
      (tester) async {
  ...
  group('OmniCastGiftingBottomSheet & Targeted Cross-Room Gifting', () {
    testWidgets(
      'renders targeted host selector when in PK mode and single button when solo',
      (tester) async {
        final roomState = RoomState();
        final signalingClient = SignalingClient();
        final streamManager = MediaStreamManager();
        ...
        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(body: OmniCastGiftingBottomSheet(client: client)),
          ),
        );
