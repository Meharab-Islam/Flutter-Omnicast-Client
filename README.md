...
## 🔑 Initialization & Auth (Clean One-Time Credentials)

Pass your server credentials directly into `OmniCastClient.init()` once (e.g. in `main.dart`). The SDK handles token generation and signaling verification internally so your UI stays clean:
...
final client = await OmniCastClient.init(
  ...
  apiKey: 'lms_7bbf0edd6408a3cd2be603d3968c10cc',
  apiSecret: 'sec_fc23871c858486a0c131090dde7a8c4a5ee6d6848ff8dd61',
  jwtSecret: '538535f0fb375b1b082292c580cb2f321aba94cb8752ed4a8bd30f36dd925567',
);
```

---

## 🎥 Quick Start (Clean UI Layer - Zero Token Boilerplate)

### 1. Join a Live Room as a Viewer
Connects with `recvonly` audio/video transceivers. The SDK automatically generates and signs the session token internally:
...
class _LiveStreamScreenState extends State<LiveStreamScreen> {
  ...
  Future<void> _initSDK() async {
    // 1. Initialize SDK (Server credentials passed once)
    ...
    // 2. Join or Host directly - the SDK signs and verifies the token internally!
    if (widget.isHost) {
      await client.room.createRoom(
