...
class SeatManager {
  ...
  // Pending seat requests list
  List<SeatRequest> get pendingSeatRequests => _roomState.pendingSeatRequests;
  ...
  void setMainSeat(String? userId) => pinToMainStage(userId);
  void restoreHostMainSeat() => pinToMainStage(null);

  /// Kicks a seat occupant back to a viewer.
  void kickSeat(dynamic target, {String? targetUserId}) {
    if (targetUserId != null && targetUserId.isNotEmpty) {
      demoteToViewer(targetUserId);
      return;
    }
    if (target is StageSeat) {
      if (target.userId != null) {
        demoteToViewer(target.userId!);
      }
    } else if (target is int) {
      final seat = _roomState.getSeat(target);
      if (seat?.userId != null) {
        demoteToViewer(seat!.userId!);
      }
    } else if (target is String) {
      demoteToViewer(target);
    }
  }
  ...
  final _seatUpdatedController = StreamController<StageSeat>.broadcast();
  ...
  Stream<StageSeat> get onSeatUpdated => _seatUpdatedController.stream;
      ...
      _signalingClient.onMessage.listen((msg) async {
        if (msg.event == SignalingEvents.seatKick ||
          ...
          if (targetUser == _roomState.userId && _roomState.isCoHost) {
            OmniCastLogger.log(
              '[SeatManager] Co-host demoted to viewer -> Teardown local tracks and switch to viewer',
            );
  ...
  void pinToMainStage(String? targetUserId) {
    ...
    _signalingClient.send(
      SignalingMessage(
        ...
        roomId: _roomState.roomId!,
        userId: _roomState.userId!,
        payload: {'pinned_user_id': targetUserId},
      ),
    );
